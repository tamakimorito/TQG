...
## 2025/12/19 - v1.3.2
- Feature: 単一のCSVを最大5件のスプレッドシートへ連続適用できるバッチ入力UIを追加（URL複数入力 + 任意タブ名指定 + 成功/失敗のシート別表示）。
- UI: 実行結果カードに合計件数とシート別の詳細・エラー表示を追加し、処理経路を可視化。
- Docs: フロントエンド仕様とバックエンドGAS v1.6.3のI/F概要を memory/spec.md に整理。
